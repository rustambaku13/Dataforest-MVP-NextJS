            ...
            $(document.body).mousemove(function( event ) {
                ...
                if(this.int == undefined){
                    for(var i=0;i<mythis.elements.length;i++){
                        var force = mythis.elements[i].mouseForce(mythis.mouseX,mythis.mouseY);
                    }
                }
              });
        }
...
class Node{
    constructor(parent,content,left,top,label){
        ...
        this.col_const = 1;
        this.hook_const = 0.043 * 3;
        this.charge =600/(parent.radius/30) * Math.sqrt(3);
        this.prev = [0,0];
        this.length = parent.parent.offsetWidth * parent.parent.offsetHeight/parent.radius/parent.radius * 3;
        this.static_charge = 10;
        this.mouse_charge = 20;
        //Charges and stuff
    ...
    coloumbAttraction(node){
        ...
        v = v.map(function(x) { return (-1) *x / d * magn; });
    ...
    hookRepelling(node){
        ...
        var v = [pos2.left-pos1.left, pos2.top-pos1.top];
        v = v.map(function(x) { return x / d * magn; });
    ...
    calculateForces(nodes,mouseX,mouseY){
        ...
        if(mouseX!=undefined){
            var statforce = this.mouseForce(mouseX,mouseY);
            resulting[0]+=statforce[0];
            resulting[1]+=statforce[1];
        }
        this.checkboundary(resulting);
    ...
    checkboundary(vector){
      
        if(this.pos.left + vector[0] > this.parent.parent.offsetWidth-this.parent.radius || this.pos.left + vector[0] < 0){
            // alert(this.pos.left);
            vector[0]=0;
        }
        ...
        if(this.pos.top + vector[1] > this.parent.parent.offsetHeight-this.parent.radius || this.pos.top + vector[1] < 20){
            // alert(this.pos.top);
            vector[1]=0;
        }
    }
    moveElement(vector){
        ...
        this.el.css("left",this.pos.left);
        this.el.css("top",this.pos.top);
